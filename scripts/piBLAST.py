#!/usr/bin/env python
# this script: pulls the seqs of query piRNA regions, blasts the seqs to the TE consensus seqeunces, and determines if the families match,
# checks whether the known Tc3 piRNAs blast to the Tc3 TE sequences
...
print "STEP 6"
#!!!
# make blast database of TE family sequences
cmd="/lscr2/andersenlab/kml436/ncbi-blast-2.2.30+/bin/makeblastdb -in  {i}_TE_seqs.fasta -dbtype nucl -out {i}_TE_seqs_database".format(**locals())
result, err = Popen([cmd],stdout=PIPE, stderr=PIPE, shell=True).communicate()

# blast piRNA seqeunces to TE sequences
cmd="/lscr2/andersenlab/kml436/ncbi-blast-2.2.30+/bin/blastn -db {i}_TE_seqs_database -query {i}_piRNAs.fasta -evalue 1 -word_size 5 -outfmt 6 -max_target_seqs 100 -out piRNA_blast.txt -num_threads 10".format(**locals())
result, err = Popen([cmd],stdout=PIPE, stderr=PIPE, shell=True).communicate()
...
# blast piRNA seqeunces to TE sequences
cmd="/lscr2/andersenlab/kml436/ncbi-blast-2.2.30+/bin/blastn -db {i}_TE_seqs_database -query {i}_full_piRNAs.fasta -evalue 1 -word_size 5 -outfmt 6 -max_target_seqs 100 -out piRNA_blast.txt -num_threads 10".format(**locals())
result, err = Popen([cmd],stdout=PIPE, stderr=PIPE, shell=True).communicate()
#!!!
...
if os.path.isfile('ONE_new_TRANS_Tc3_TE_seqs.fasta'):
	...
	# make blast database of TE family sequences
	cmd="/lscr2/andersenlab/kml436/ncbi-blast-2.2.30+/bin/makeblastdb -in  ONE_new_TRANS_Tc3_TE_seqs.fasta -dbtype nucl -out Tc3_database".format(**locals())
	result, err = Popen([cmd],stdout=PIPE, stderr=PIPE, shell=True).communicate()

	# blast piRNA seqeunces to TE sequences
	cmd="/lscr2/andersenlab/kml436/ncbi-blast-2.2.30+/bin/blastn -db Tc3_database -query {known_Tc3_pi} -evalue 1 -word_size 5 -outfmt 6 -max_target_seqs 100 -out piRNA_blast.txt -num_threads 10".format(**locals())
	result, err = Popen([cmd],stdout=PIPE, stderr=PIPE, shell=True).communicate()
