...
result, err = Popen(["""cat *.bed > summary_depth_TPFD.txt"""], stdout=PIPE, stderr=PIPE, shell=True).communicate()
result, err = Popen(["""echo '{header}' | cat - summary_depth_TPFD.txt > temp && mv temp summary_depth_TPFD.txt""".format(**locals())], stdout=PIPE, stderr=PIPE, shell=True).communicate()
