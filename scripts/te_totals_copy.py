#!/usr/bin/env python 
#this script processes TEMP output to calculate how many transposons of each family  and totals were found in a strain
...
for line in TE_OUTPUT:
	...
	if family in consensus_families.keys():
		...
		print "ERROR: The detected family '{family}' does not exist in the consensus file, exiting...".format(**locals())
		RESULTS_FILE.write("{date}\t{bam_name}\t{method}\tERROR\n".format(**locals()))
		sys.exit()

TE_OUTPUT.close()
RESULTS_FILE.write("{date}\t{bam_name}\t{method}\t{end_tes}\t{total_tes}\t".format(**locals()))
RESULTS_FILE_LF.write("{date}\t{bam_name}\t{method}_T:total\t{total_tes}\n".format(**locals()))
RESULTS_FILE_LF.write("{date}\t{bam_name}\t{method}_T:end_tes\t{end_tes}\n".format(**locals()))


for key in sorted(consensus_families.keys()):
	count = consensus_families[key]
	RESULTS_FILE.write("{count}\t".format(**locals()))
	RESULTS_FILE_LF.write("{date}\t{bam_name}\t{method}_T:{key}\t{count}\n".format(**locals()))
RESULTS_FILE.write("\n")
