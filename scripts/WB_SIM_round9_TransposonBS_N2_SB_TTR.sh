...
# this script runs round9 of the transposon simulations/detection
# USE: WB_SIM_round9_TransposonsBS_N2_SB_TTR.sh
...
##############################################################
echo "Running TEMP..."
bash ${TTR}/TEMP/scripts/TEMP_Insertion.sh -i ${run_ID}_${bam_name}.sorted.bam -s ${TTR}/TEMP/scripts/ -x 30 -r $TE_consensus -t $HL_gff -m 27 -c 20 &> ${run_ID}_TEMP_log.txt
