...
sed '1d' "${bam_name}.insertion.refined.bp.summary" | awk '{if ($4 == "sense" || $4 == "antisense"); else print $0}' | awk '{ printf $1"\t"$9"\t"$11"\t"$7"\t"$4"_non-reference_"sample"_temp_\t0\t"; if ( $5 == "sense" ) printf "+"; else printf "-"; print "\t"$6"\t"$10"\t"$12}' > "${bam_name}_temp_presort_raw.txt"
bedtools sort -i "${bam_name}_temp_presort_raw.txt" > "${bam_name}_temp_sorted_raw.txt"
awk '{ printf $1"\t"$2"\t"$3"\t"$4"\t"$5; if ($9 > 0 && $10 > 0) printf "sr_"; else printf "rp_"; print NR"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10}' "${bam_name}_temp_sorted_raw.txt" > "${bam_name}_temp_sorted_precut_raw.txt"
awk '{ print $1"\t"$2"\t"$3"\t"$5"\t"$4"\t"$7}' "${bam_name}_temp_sorted_precut_raw.txt" >> "${bam_name}_temp_raw.bed"
...
sort -k1,3 -k4rn "${bam_name}_temp_sorted_redundant.txt" | sort -u -k1,3 | awk '{ print $1"\t"$2"\t"$3"\t"$5"\t"$4"\t"$7}' > "${bam_name}_tmp"
...
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/correct_names_2_set2.py $family_renames ${bam_name}_temp_nonredundant.bed TMP
...
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/collapse_transposons.py $TE_lengths ${bam_name}_temp_nonredundant.bed
cp ${bam_name}_temp_nonredundant.bed ../final_results/${bam_name}_temp_insertion_nonredundant.bed
...
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/te_totals.py new_CT_${bam_name}_temp_nonredundant.bed $consensus_renamed ${bam_name} $results_file new $end_tes
cd ..
...
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/process_double_deletion.py ${bam_name}_temp_nonredundant.bed $ref_tes
...
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/te_totals.py ${bam_name}_temp_nonredundant.bed $consensus_renamed ${bam_name} $results_file absent $end_tes
cd ..
...
sort -k1,3 -k4rn "${bam_name}_tmpfile_telocate_counted_redundant.txt"| sort -u -k1,3 | awk '{ print $1"\t"$2"\t"$3"\t"$5"\t"$4"\t"$7}'> "${bam_name}_tmpfileNR"
...
# RENAME TRANSPOSON CALLS
#rename the TEs in the output tp match final consensus names
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/correct_names_2_set2.py $family_renames ${bam_name}_telocate_nonredundant.bed TMP
...
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/process_telocate_output.py ${bam_name}_closest.txt
...
python /lscr2/andersenlab/kml436/git_repos2/Transposons/scripts/te_totals.py ${bam_name}_telocate_nonredundant.bed  $consensus_renamed ${bam_name} $results_file reference $end_tes
cd ..
