...
# promoter here is defined as the region 1000 bp upstream of the start of the gene, or up until the closest feature if that feature is within 1000 bps
...
promoters="{file_dir}/WB_promoters.gff".format(**locals())
...
os.system("bedtools closest -a {plus_minus} -b {points} -id -D a -t all -k 2 >gene_to_gene.txt".format(**locals()))
...
with open("gene_to_gene.txt", 'r') as IN:
	for line in IN:
		...
		if seq != matched_seq:
			if distance <= -1000: # if more than 1000 bp away
				if orient == "+":
					promoter_start=start1-1000	
				if orient == "-":
					promoter_start=start1+1000

			if distance > -1000: # if more than 1000 bp away
				if orient == "+":
					promoter_start=start1+distance	
				if orient == "-":
					promoter_start=start1-distance
			if seq in promoter_regions.keys(): # if duplicate transcript name (occurs in 3 cases), take the earlier start position
