...
for i in run_list:
	...
	sc_file="{my_dir}/run_{run}/run_{run}_filter_results_telocate/intermediates_RECALCULATED/SC_new_CT_run_{run}_telocate_nonredundant.bed_to_adjusted_pos_REF_tes_{run}.bed_compare.bed".format(**locals())
	SC=open(sc_file, "r")
	true_positives={}
	true_pos_position={}
...
result, err = Popen(["""echo '{header}' | cat - summary_depth_TPFD_ref.txt > temp && mv temp summary_depth_TPFD_ref.txt""".format(**locals())], stdout=PIPE, stderr=PIPE, shell=True).communicate()
result, err = Popen([""" cat summary_depth_TPFD_ref.txt summary_depth_TPFD_FN.txt > temp && mv temp summary_depth_TPFD_ref.txt""".format(**locals())], stdout=PIPE, stderr=PIPE, shell=True).communicate()
